...
from models import db, Admin, AdminSession, Player, PlayerSession, Challenge, Match, Tournament, TournamentParticipant, PlayerStatus, ChallengeStatus, MatchStatus, TournamentStatus
from app.auth import require_admin_auth, require_player_auth, authorize_player_action, get_authenticated_user
...
# For TESTING purposes, serve a simple HTML page
@bp.route('/')
def index():
...
@bp.route('/player/login', methods=['POST'])
def player_login():
    data = request.json
    
    valid, error_response, status_code = validate_required_fields(data, ['name', 'password'])
    if not valid:
        return error_response, status_code
    ...
    if player.status != PlayerStatus.APPROVED:
        return jsonify({'error': 'Player account is not approved yet'}), 403
    
    # Clean up expired sessions
    PlayerSession.query.filter(PlayerSession.expires_at < datetime.now()).delete()
    
    # Create new session
    session = PlayerSession(player_id=player.id)
    db.session.add(session)
...
@bp.route('/player/logout', methods=['POST'])
def player_logout():
...
def register_player():
    ...
    valid, error_response, status_code = validate_required_fields(data, ['name', 'age', 'weight', 'password'])
    if not valid:
        return error_response, status_code
    
    name, age, weight, password = data.get('name'), data.get('age'), data.get('weight'), data.get('password')
    ...
    player = Player(name=name, age=age, weight=weight, password_hash='')
...
@bp.route('/admin/players/<int:player_id>/reject', methods=['DELETE'])
def reject_player(player_id):
    ...
    # Instead of setting status to REJECTED, we delete the player from database
    db.session.delete(player)
    success, error = safe_commit()
    if not success:
        return error
    
    return jsonify({'message': 'Player rejected and removed from database'})
...
@bp.route('/players/weight', methods=['PUT'])
def update_weight():
    """Update weight for authenticated player or admin can specify player_id"""
    data = request.json
    ...
    # Check if admin is specifying a player_id
    user, user_type = get_authenticated_user()
    if not user:
        return jsonify({'error': 'Authentication required'}), 401
    
    if user_type == 'admin':
        # Admin must specify player_id
        player_id = data.get('player_id')
        if not player_id:
            return jsonify({'error': 'Admin must specify player_id'}), 400
        player, error = authorize_player_action(player_id)
        if error:
            return error
    else:
        # Player updates their own weight
        player = user
    
    if not player.is_active():
        return jsonify({'error': 'Cannot update weight for inactive player'}), 400
...
def create_challenge():
    """Create a challenge - must be authenticated as one of the players or as admin"""
    ...
    # Authorize that the logged-in user is the challenger
    authorized_player, error = authorize_player_action(challenger_id)
    if error:
        return error
...
def accept_challenge(challenge_id):
    """Accept a challenge - must be authenticated as the challenged player or admin"""
    cleanup_expired_challenges()
    
    challenge = Challenge.query.get_or_404(challenge_id)
    
    # Authorize that the logged-in user is the challenged player
    authorized_player, error = authorize_player_action(challenge.challenged_id)
    if error:
        return error
    ...
    # Challenge is accepted, create match immediately
...
def create_tournament():
    """Create a tournament - must be authenticated as the host or admin"""
    data = request.json
    ...
    # Authorize that the logged-in user is the host
    authorized_player, error = authorize_player_action(host_id)
    if error:
        return error
    ...
    if not authorized_player.is_active():
...
def join_tournament(tournament_id):
    """Join a tournament - must be authenticated as the joining player or admin"""
    update_tournament_status()
    
    tournament = Tournament.query.get_or_404(tournament_id)
    data = request.json
    
    # Get player_id from request or from authenticated player
    user, user_type = get_authenticated_user()
    if not user:
        return jsonify({'error': 'Authentication required'}), 401
...
def leave_tournament(tournament_id):
    """Leave a tournament - must be authenticated as the leaving player or admin"""
    update_tournament_status()
    ...
    if user_type == 'admin':
        ...
        # Player leaves themselves
        player_id = user.id
    
    # Authorize the action
    _, error = authorize_player_action(player_id)
    if error:
        return error
...
def record_tournament_match(tournament_id):
    """Host records a match result between any two tournament participants - must be authenticated as host or admin"""
    update_tournament_status()
    ...
    # Authorize that the logged-in user is the host
    host, error = authorize_player_action(host_id)
    if error:
        return error
...
@bp.route('/matches/result', methods=['POST'])
def record_match_result():
    """Record match result - must be authenticated as the host or admin"""
    ...
    # Find the first PENDING match between the two players
    match = Match.query.filter(
        Match.status == MatchStatus.PENDING,
        db.or_(
            db.and_(Match.player1_id == player1_id, Match.player2_id == player2_id),
            db.and_(Match.player1_id == player2_id, Match.player2_id == player1_id)
        )
    ).order_by(Match.created_at.asc()).first()
    
    if not match:
        return jsonify({'error': 'No pending match found between these players'}), 404
...
def undo_last_match():
    """Undo the last match recorded by a host - must be authenticated as that host or admin"""
    data = request.json
    
    # Get host_id from request or from authenticated player
    user, user_type = get_authenticated_user()
    if not user:
        return jsonify({'error': 'Authentication required'}), 401
    ...
    if user_type == 'admin':
        ...
        if not host_id:
            return jsonify({'error': 'Admin must specify host_id'}), 400
    else:
        # Player undoes their own hosted match
        host_id = user.id
    ...
    last_match = Match.query.filter_by(
        ...
        Match.status.in_([MatchStatus.COMPLETED, MatchStatus.UNDONE])
    ).order_by(Match.completed_at.desc()).first()
    
    if not last_match:
        return jsonify({'error': 'No completed match found for this host to undo'}), 404
    if last_match.status == MatchStatus.UNDONE:
        return jsonify({'error': 'Cannot record new match: last match was undone'}), 400
...
def run_sql():
    ...
    for keyword in dangerous_keywords:
        if keyword in query_upper:
            return jsonify({'error': f'Operation {keyword} is not allowed for data integrity'}), 403
